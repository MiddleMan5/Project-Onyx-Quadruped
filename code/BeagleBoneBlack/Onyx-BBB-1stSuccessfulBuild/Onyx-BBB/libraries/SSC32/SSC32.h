...
class SSC32
{
	...
	public:
		...
		void begin(int br);
		void check();
		bool newGroup(int type);
		bool abortGroup();
		bool executeGroup();

		bool servoMove(int channel, int position);
		bool servoMove(int channel, int position, int speed);
		bool servoMoveTime(int channel, int position, int moveTime);

		bool offset(int channel, int offset);
		bool discrete(int channel, bool level);
		bool byte(int bank, int value);
