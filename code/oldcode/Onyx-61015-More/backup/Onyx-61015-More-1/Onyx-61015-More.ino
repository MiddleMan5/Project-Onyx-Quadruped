...
//coxa-femur-tibia-foot/claw
        ...
        //4 group spaces to keep track of X and Y Values
        //    X or Y          Group == 1     2     3     4
        float prevGroupX[4];//Multi-state logging test
        float prevGroupY[4];
        float prevx,prevy;
        ...
        float nextGroupX[4];//Multi-state logging test
        float nextGroupY[4];
        float nextx,nexty;

////Populated on startup, 12 Servo 3 posistion frame array1s:
int ServoPosistion[12]; //legacy1 
...
IK(BodyX,BodyY);
    ...
    ServoNext[11] = pulse2;
      ...
      for(int i=0;i<12;i++){ //Records updated posisiton as the now-cuurent posistions
